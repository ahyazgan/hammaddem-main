...
export interface SehirBilgi {
  slug: string;
  ad: string;
  /** Bulunma hali: İstanbul'da, İzmir'de, Kocaeli'nde */
  adDa: string;
  /** İlgi hali: İstanbul'un, İzmir'in, Kocaeli'nin */
  adIn: string;
  osblar: string[];
  ilceler: string[];
  sanayi: string;
...
export const MALZEME_ICERIK: Record<string, MalzemeKomboIcerik> = {
  cimento: {
    ...
    faq: (s) => [
      {
        q: `${s.adDa} çimento silobas taşıma fiyatları nasıl belirlenir?`,
        a: `${s.adDa} çimento fiyatları; ton miktarı, teslimat noktası (${s.ilceler[0]}, ${s.ilceler[1]} gibi ilçeler arasında mesafeye göre farklılık gösterir), çimento türü (CEM I, CEM II) ve teslimat zamanlamasına göre değişir. Hammaddem üzerinden talep oluşturduğunuzda 30 dakika içinde güncel fiyat teklifi alırsınız.`,
      },
      {
      ...
      {
        q: `${s.adDa} aynı gün çimento teslimatı mümkün mü?`,
  ...
  kum: {
    aciklama: (s) =>
      `${s.adDa} inşaat kumu ve dere kumu tedariki için Hammaddem'i kullanın. ${s.inşaatBolgeleri} başta olmak üzere ${s.adIn} tüm yapılaşma bölgelerine kum teslimatı yapıyoruz. Yıkanmış kum, kırma kum ve dere kumu çeşitleri için ton bazında rekabetçi fiyat; ${s.lojistik} imkânıyla hızlı teslimat garantisi sunuyoruz.`,
    hizmetDetay: (s) =>
    ...
    yerelBilgi: (s) =>
      `${s.adDa} kum ihtiyacı en yoğun ${s.ilceler.slice(0, 3).join(", ")} ilçelerinde faaliyet gösteren müteahhit ve yapı firmalarına öncelikli hizmet veriyoruz. ${s.inşaatBolgeleri} bölgeleri için stok durumuna göre aynı gün kum teslimatı organize edilebilmektedir.`,
    faq: (s) => [
      {
        q: `${s.adDa} inşaat kumu çeşitleri ve fiyatları nasıldır?`,
        a: `${s.adDa} dere kumu, yıkanmış kum ve kırma kum çeşitlerinde hizmet veriyoruz. Fiyatlar; ${s.ilceler[0]} ve ${s.ilceler[1]} gibi bölgelere göre nakliye mesafesiyle değişir, ton başına rakibimizden %10-15 daha uygun fiyat sunmayı hedefliyoruz. Güncel fiyat için talep oluşturun.`,
      },
      {
      ...
      {
        q: `${s.adDa} hangi kum kalitesi inşaat için uygundur?`,
        a: `${s.adDa}ki zemin ve iklim koşulları için genellikle yıkanmış veya elekli kum önerilir. Beton üretimi için CEN standartlarına uygun kırma kum, sıva ve dolgu için dere kumu tercih edilmektedir. Teknik destek için Hammaddem uzmanlarına ulaşabilirsiniz.`,
      },
    ],
  },
  ...
  cakil: {
    ...
    yerelBilgi: (s) =>
      `${s.adDa} çakıl kullanımı en yoğun ${s.inşaatBolgeleri} bölgelerindedir. ${s.osblar[0]} sanayi bölgesi firmalarına agrega ve çakıl tedariki konusunda uzun süreli deneyime sahibiz. ${s.ilceler.slice(0, 2).join(" ve ")} ilçelerindeki altyapı projelerine öncelikli hizmet veriyoruz.`,
    faq: (s) => [
      {
        q: `${s.adDa} çakıl fiyatları ton başına ne kadardır?`,
        a: `${s.adDa} çakıl fiyatları agrega boyutuna (0-5mm, 5-12mm, 12-25mm), mesafeye ve miktara göre değişmektedir. ${s.ilceler[0]} merkezine ton başına nakliye dahil fiyat almak için Hammaddem üzerinden talep oluşturun; 30 dakika içinde güncel fiyat bildiriyoruz.`,
      },
      {
        q: `${s.adDa} beton agregası için hangi çakıl boyutu uygundur?`,
        a: `${s.adDa}ki standart beton üretimi için 5-12mm ve 12-25mm kırmataş agrega kullanılır. Drenaj ve temel dolgusunda 25-50mm iri çakıl tercih edilmektedir. Üretim standardına göre doğru malzeme seçimi için teknik ekibimizle iletişime geçebilirsiniz.`,
      },
      {
        q: `${s.ad} bölgesine büyük tonajlı çakıl siparişi verebilir miyim?`,
        a: `Evet, ${s.adDa} 100 ton ve üzeri büyük hacimli çakıl siparişleri için özel fiyat ve uzun vadeli tedarik sözleşmeleri düzenlenebilmektedir. ${s.osblar[0]} ve ${s.inşaatBolgeleri.split(",")[0]} gibi büyük proje bölgelerine toplu teslimat koordinasyonu sağlıyoruz.`,
      },
    ],
  },
  ...
  micir: {
    aciklama: (s) =>
      `${s.adDa} mıcır ve kırmataş tedariki için Hammaddem platformunu kullanın. ${s.sanayi} kapsamındaki sanayi tesisleri, beton santralleri ve altyapı projelerine ${s.osblar.slice(0, 3).join(", ")} dahil tüm ${s.ad} bölgesine mıcır teslimatı yapıyoruz. 0-5mm, 4-8mm ve 8-16mm ebatlarında kaliteli mıcır için rekabetçi fiyat teklifi alın.`,
    hizmetDetay: (s) =>
      `${s.adDa} mıcır tedarik sürecini Hammaddem dijitalleştiriyor. Talep formunda mıcır boyutunu ve miktarı belirttiğinizde ${s.adDa}ki tedarikçi ağımız 30 dakika içinde fiyat teklifi sunar. ${s.lojistik} altyapısı sayesinde ${s.ilceler.slice(0, 4).join(", ")} ilçelerinde aynı gün veya ertesi gün teslimat mümkündür. Düzenli mıcır ihtiyaçlarınız için haftalık veya aylık programlı teslimat seçeneği de sunuyoruz.`,
    yerelBilgi: (s) =>
      `${s.adDa} mıcır en çok ${s.inşaatBolgeleri} bölgelerindeki altyapı ve inşaat projelerinde kullanılmaktadır. ${s.osblar[0]} sanayi bölgesindeki beton santralleri ve üretim tesisleri başlıca mıcır kullanıcılarımız arasındadır. ${s.ilceler.slice(1, 3).join(" ve ")} ilçelerindeki yol yapım projelerine de düzenli teslimat yapıyoruz.`,
    faq: (s) => [
      {
        q: `${s.adDa} mıcır boyutları ve fiyatları nelerdir?`,
        a: `${s.adDa} 0-5mm ince mıcır (şap için), 4-8mm (beton için), 8-16mm ve 16-32mm kaba mıcır çeşitleri mevcuttur. Fiyatlar ${s.ilceler[0]} merkezine göre değişmekte olup ton başına fiyat için Hammaddem'den talep oluşturun.`,
      },
      {
        q: `${s.adDa} mıcır siparişi için asgari miktar nedir?`,
      ...
      },
      {
        ...
        a: `Evet, ${s.adDa} tedarik ettiğimiz kırma mıcır, karayolları teknik şartnamesine uygun standartlarda olmaktadır. ${s.ilceler.slice(0, 2).join(" ve ")} ilçelerindeki belediye ve özel yol projelerinde sık tercih edilmektedir.`,
      },
    ],
  },
  ...
  kalsit: {
    ...
    yerelBilgi: (s) =>
      `${s.adDa} kalsit kullanımı ${s.osblar[0]} ve ${s.osblar[1] ?? s.osblar[0]} sanayi bölgelerindeki boya, plastik ve yapı kimyasalları üreticilerinde yoğunlaşmaktadır. ${s.inşaatBolgeleri} bölgelerindeki boya ve kaplama firmalarına da düzenli kalsit tedariki yapıyoruz.`,
    faq: (s) => [
      {
        q: `${s.adDa} kalsit çeşitleri ve kullanım alanları nelerdir?`,
        a: `${s.adDa} GCC (öğütülmüş) ve PCC (çöktürülmüş) kalsit çeşitleri mevcuttur. Boya ve plastik sektöründe 3-5 mikron, kağıt sektöründe 1-2 mikron, yapı malzemeleri ve sıva sektöründe 40-100 mikron kalsit kullanılmaktadır. ${s.osblar[0]} firmalarına özel granülometri seçenekleri sunuyoruz.`,
      },
      {
        q: `${s.adDa} kalsit silobas taşıma nasıl yapılır?`,
        a: `${s.adDa} kalsit, nem ve kirlenmeyi engellemek için pnömatik silobas araçlarla taşınır. ${s.ilceler[0]} ve çevresindeki üretim tesislerine silobas boşaltma hattı uygunluğuna göre doğrudan silolara aktarım yapıyoruz.`,
      },
      {
        q: `${s.ad} bölgesinde kalsit minimum sipariş miktarı nedir?`,
        a: `${s.adDa} minimum 10 ton kalsit siparişi kabul ediyoruz. ${s.osblar[0]} bölgesindeki düzenli tedarik anlaşmaları için aylık ve üç aylık sözleşme seçenekleri mevcuttur.`,
      },
    ],
  },
  ...
  kirec: {
    aciklama: (s) =>
      `${s.adDa} kireç (söndürülmüş/sönmemiş) tedariki için Hammaddem platformunu kullanın. ${s.sanayi} sektörlerine hizmet veren ${s.osblar.slice(0, 2).join(" ve ")} başta olmak üzere ${s.ad} genelinde kireç teslimatı yapıyoruz. Atık su arıtma, demir-çelik, inşaat ve tarım sektörü için CaO ve Ca(OH)₂ türündeki kireç ürünlerini silobas araçlarla güvenle taşıyoruz.`,
    hizmetDetay: (s) =>
      `${s.adDa} kireç tedarik sürecini Hammaddem hızlandırıyor. Talep formunda kireç türü (CaO/Ca(OH)₂), saflık derecesi ve miktarı belirttiğinizde ${s.ad} bölgesi için 30 dakika içinde fiyat teklifi alırsınız. ${s.lojistik} sayesinde ${s.ilceler.slice(0, 3).join(", ")} ilçelerindeki arıtma tesisleri ve üretim santrallerine zamanında kireç teslimatı garanti ediyoruz.`,
    yerelBilgi: (s) =>
      `${s.adDa} kireç en fazla ${s.osblar[0]} bölgesindeki demir-çelik ve metal işleme tesisleri ile atık su arıtma tesislerinde kullanılmaktadır. ${s.inşaatBolgeleri} bölgelerindeki sıva ve bağlayıcı malzeme olarak da söndürülmüş kireç talebimiz yüksektir.`,
    faq: (s) => [
      {
        q: `${s.adDa} sönmemiş kireç (CaO) ile söndürülmüş kireç (Ca(OH)₂) farkı nedir?`,
        a: `Sönmemiş kireç (CaO) demir-çelik, çevre arıtma ve kimya sanayisinde kullanılırken; söndürülmüş kireç Ca(OH)₂ tarım, inşaat sıvası ve su arıtmada tercih edilir. ${s.adDa}ki ${s.osblar[0]} gibi sanayi tesisleri genellikle sönmemiş kireç kullanmaktadır.`,
      },
      {
      ...
      {
        q: `${s.adDa} kireç teslim süresi ne kadardır?`,
  ...
  alci: {
    ...
    yerelBilgi: (s) =>
      `${s.adDa} alçı kullanımı ${s.inşaatBolgeleri} bölgelerindeki inşaat projelerinde yoğunlaşmaktadır. ${s.osblar[0]} ve çevresindeki alçı ürünleri üreticilerine düzenli hammadde tedariki yapıyoruz.`,
    faq: (s) => [
      {
        q: `${s.adDa} beta ve alfa alçısı farkı ve kullanım alanları nelerdir?`,
        a: `Beta alçısı (hemihydrate β) inşaat sıvası, alçı levha ve prefabrik üretimde; alfa alçısı ise diş alçısı, tıp ve yüksek mukavemetli seramik kalıp üretiminde kullanılır. ${s.adDa}ki inşaat sektörü ağırlıklı olarak beta alçısı talep etmektedir.`,
      },
      {
        q: `${s.ad} bölgesinde alçı silobas taşıma nasıl çalışır?`,
        a: `${s.adDa} alçı silobas araçlarla nem almadan taşınır, depolama silolarına pnömatik olarak aktarılır. ${s.ilceler[0]} ve ${s.ilceler[1]} bölgesindeki şantiye ve üretim tesislerine uyumlu boşaltma sistemiyle hizmet veriyoruz.`,
      },
      {
        q: `${s.adDa} alçı fiyatları nasıl şekilleniyor?`,
        a: `${s.adDa} alçı fiyatları; saflık derecesi, incelik (mesh), nakliye mesafesi ve sipariş miktarına göre değişmektedir. ${s.osblar[0]} bölgesine ton başına fiyat için Hammaddem'den talep oluşturun, 30 dakika içinde güncel fiyat alırsınız.`,
      },
    ],
  },
  ...
  "mermer-tozu": {
    aciklama: (s) =>
      `${s.adDa} mermer tozu ve kalsit unu tedariki için Hammaddem'i kullanın. ${s.sanayi} kapsamındaki üretim tesisleri başta olmak üzere ${s.osblar.slice(0, 2).join(" ve ")} bölgelerine mermer tozu teslimatı yapıyoruz. Boya, plastik, dolgu malzemesi ve tarım sektörü için farklı incelik derecelerinde mermer tozu ve kalsit unu tedarik ediyoruz.`,
    hizmetDetay: (s) =>
      `${s.adDa} mermer tozu tedarik hizmetimiz silobas araçlarla kapalı sistem taşıma imkânı sunar. ${s.lojistik} altyapısı sayesinde ${s.ilceler.slice(0, 3).join(", ")} ilçelerindeki boya, yapı kimyasalları ve plastik üretim tesislerine zamanında teslimat yapıyoruz. Hammaddem platformunda talep oluşturun, 30 dakika içinde ${s.ad} için fiyat teklifi alın.`,
    yerelBilgi: (s) =>
      `${s.adDa} mermer tozu tüketimi ${s.osblar[0]} ve ${s.osblar[1] ?? s.osblar[0]} bölgelerindeki boya ve yapı malzemeleri üretim tesislerinde yoğunlaşmaktadır. ${s.inşaatBolgeleri} bölgelerindeki cephe kaplaması ve dekoratif beton üretiminde de artan mermer tozu talebi karşılıyoruz.`,
    faq: (s) => [
      {
        q: `${s.adDa} mermer tozu incelik dereceleri ve kullanım alanları nelerdir?`,
        a: `${s.adDa} 40 mesh (boya dolgu), 100 mesh (plastik dolgu), 200-325 mesh (ince boya ve kaplama) mermer tozu çeşitleri tedarik ediyoruz. ${s.osblar[0]} bölgesindeki fabrikalar için teknik şartnameye uygun granülometride teslimat yapıyoruz.`,
      },
      {
        q: `${s.ad} bölgesine mermer tozu silobas taşıma imkânı var mı?`,
        a: `Evet, ${s.adDa}ki ${s.ilceler[0]} ve ${s.ilceler[1]} bölgelerindeki üretim tesislerine mermer tozu pnömatik silobas araçlarla teslim edilmektedir. Silo kapasitesine göre farklı araç büyüklüklerinde hizmet verilmektedir.`,
      },
      {
        q: `${s.adDa} boya sektörü için en çok hangi mermer tozu tercih edilir?`,
        a: `${s.adDa}ki boya üreticileri genellikle 40-100 mesh arası, beyazlık değeri yüksek (L>95) GCC mermer tozu tercih etmektedir. ${s.osblar[0]} bölgesindeki boya fabrikalarının ihtiyaçları doğrultusunda uygun saflık ve incelikteki mermer tozu tedariki yapıyoruz.`,
      },
    ],
  },
  ...
  "ucucu-kul": {
    ...
    yerelBilgi: (s) =>
      `${s.adDa} uçucu kül kullanımı ${s.osblar[0]} bölgesindeki beton santralleri ve çimento fabrikalarında yoğunlaşmaktadır. ${s.inşaatBolgeleri} bölgelerindeki büyük altyapı projelerinde uçucu kül kullanımı zemin dayanımını artırırken maliyeti düşürmektedir.`,
    faq: (s) => [
      {
        q: `${s.adDa} uçucu kül hangi sektörlerde kullanılır?`,
        a: `${s.adDa} uçucu kül başlıca hazır beton ve beton santrallerinde çimento ikamesi (beton dayanımını artırır), zemin iyileştirme ve stabilizasyon, tuğla ve yapı bloku üretiminde kullanılmaktadır. ${s.osblar[0]} bölgesindeki beton santralleri başlıca kullanıcılarımız arasındadır.`,
      },
      {
        q: `${s.adDa} F sınıfı ile C sınıfı uçucu kül farkı nedir?`,
        a: `F sınıfı uçucu kül (düşük kireç, pozolan) dayanım artırıcı beton katkısı; C sınıfı uçucu kül (yüksek kireç) ise zemin stabilizasyonu ve kendiliğinden bağlayıcı karışımlar için uygundur. ${s.adDa}ki proje ihtiyacınıza göre doğru sınıfı öneriyoruz.`,
      },
      {
  ...
  stabilize: {
    ...
    yerelBilgi: (s) =>
      `${s.adDa} stabilize en fazla ${s.inşaatBolgeleri} bölgelerindeki OSB iç yolları, konut sitesi girişleri ve kırsal yol iyileştirme projelerinde kullanılmaktadır. ${s.osblar[0]} bölgesindeki yol yapım ve bakım çalışmalarına düzenli stabilize tedariki sağlıyoruz.`,
    faq: (s) => [
      {
        q: `${s.adDa} stabilize malzeme çeşitleri nelerdir?`,
        a: `${s.adDa} kırmataş stabilize (0-31mm), kırma kum stabilize ve doğal zemin malzemesi çeşitleri mevcuttur. OSB yolları için genellikle 0-31mm kırmataş stabilize, geçici şantiye yolları için ise daha kaba granülometri tercih edilir.`,
      },
      {
        q: `${s.adDa} stabilize yol zemini kaç cm uygulanır?`,
      ...
      },
      {
        ...
        a: `${s.adDa} 500 ton ve üzeri stabilize siparişleri için Hammaddem'de özel talep formu doldurulabilir. ${s.osblar[0]} ve çevresindeki büyük altyapı projelerine haftalık programlı teslimat koordinasyonu sağlıyoruz.`,
      },
    ],
  },
};
