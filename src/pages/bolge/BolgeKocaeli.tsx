...
const BolgeKocaeli = () => {
  const digerSehirler = BOLGE_ROUTES.filter((r) => r.path !== "/hizmet-bolgeleri/kocaeli");
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:image" content="https://hammaddem.co/og/il-kocaeli.jpg" />
      ...
      <div className="min-h-screen bg-dot-pattern">
        ...
        <section className="py-16 px-4 md:px-10 bg-off">
          <div className="max-w-[1100px] mx-auto">
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight mb-6">Kocaeli&apos;da Hizmetlerimiz</h2>
            <div className="prose prose-sm max-w-none text-txt-2 leading-[1.8] space-y-4">
              <p>Kocaeli, İzmit Körfezi çevresindeki yoğun sanayi kuşağıyla Türkiye'nin en kritik hammadde tedarik bölgelerinden biridir. GOSB, Dilovası OSB ve Gebze'deki petrokimya, kimya ve üretim tesislerine düzenli silobas ve hafriyat hizmeti sunuyoruz.</p>
              <p>Hammaddem ile Kocaeli'nde çimento, kalsit, kireç ve diğer toz malzemeler için silobas taşımacılığı; kum, çakıl ve mıcır için hafriyat tedariği hizmetlerine dijital platformdan ulaşın. Gebze, Dilovası ve GOSB bölgesine aynı gün teslimat imkânı.</p>
            </div>
          </div>
        </section>
        ...
        <section className="py-16 px-4 md:px-10">
          <div className="max-w-[1100px] mx-auto">
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight mb-6">Kocaeli Hizmet Detayı</h2>
            <div className="grid md:grid-cols-2 gap-5">
              <div className="border border-border rounded-2xl p-6 bg-background hover:border-accent-border transition-colors">
                ...
                <p className="text-sm text-txt-2 leading-relaxed mb-2">
                  Çimento, kalsit, uçucu kül, kireç, mermer tozu, alçı ve tüm toz/granül malzemeleri pnömatik silobas araçlarıyla Kocaeli genelinde teslim ediyoruz.
                </p>
                <p className="text-xs text-txt-2 leading-relaxed mb-3">
                  <strong>Hizmet bölgeleri:</strong> GOSB, Dilovası OSB, Çayırova, Gebze OSB
                </p>
        ...
        <section className="py-16 px-4 md:px-10 bg-off">
          <div className="max-w-[1100px] mx-auto">
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight mb-3">Kocaeli&apos;da Taşıdığımız Malzemeler</h2>
            <p className="text-sm text-txt-2 mb-8 max-w-[520px]">Detay ve fiyat için malzeme sayfasına tıklayın, Kocaeli&apos;a özel teklif alın.</p>
            <div className="flex flex-wrap gap-2.5">
              {MALZEME_ROUTES.map((m) => (
                <Link key={m.path} to={`/malzeme/${m.path.split("/").pop()}/kocaeli`} className="px-4 py-2 rounded-full text-sm font-medium bg-background border border-border hover:border-accent-border hover:text-primary transition-colors no-underline">
                  Kocaeli — {m.label}
                </Link>
              ))}
            </div>
